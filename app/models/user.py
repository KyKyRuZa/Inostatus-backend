...
class User(Base):
    ...
    media_outlets = Column(Text, nullable=True)  # СМИ / Издательства

    api_keys = relationship("APIKey", back_populates="user", cascade="all, delete-orphan")
    check_history = relationship("CheckHistory", back_populates="user", cascade="all, delete-orphan")
...
class APIKey(Base):
    ...
    name = Column(String(255), nullable=True)
    key_type = Column(String(50), default="free", nullable=False)  # free, long, short, package
    max_uses = Column(Integer, default=2, nullable=False)  # Максимальное количество использований
    used_count = Column(Integer, default=0, nullable=False)  # Сколько уже использовано
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.utcnow)
...
class CheckHistory(Base):
    ...
    __table_args__ = (
        CheckConstraint('similarity_score >= 0 AND similarity_score <= 100', name='check_similarity_score_range'),
    )
    ...
    result = Column(Text, nullable=True)  # JSON результат проверки
