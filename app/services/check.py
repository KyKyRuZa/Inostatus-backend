...
async def check_text_fragment(text: str, filename: str = "site_fragment_1", api_key: str | None = None) -> dict:
    import logging
    ...
    key_display = f"{effective_api_key[:8]}...{effective_api_key[-4:]}" if effective_api_key else "NONE"
...
async def wait_for_task_completion(
    ...
    while elapsed < timeout:
        ...
        if status_data["status"] == "completed":
            ...
            raise TaskStatusError(f"Ошибка обработки: {status_data.get('error', 'Неизвестная ошибка')}")
